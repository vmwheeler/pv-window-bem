...
# Round EQEQ to make really small numbers equal to zero
EQEs = EQEs.round(8)
AbsInterp = scipy.interpolate.interp1d(lams, EQEs)
...
def AbsByAbsorber(Ephoton):
    λ = hPlanck * c0 / Ephoton 
    return AbsInterp(λ)
#return AbsInterp(λ) * (1 / Ephoton) * (hPlanck * c0 / Ephoton**2)
    
def EQEspec(eta,Ephoton):
    return eta * AbsByAbsorber(Ephoton)

def RR0(eta):
    integrand = lambda E : EQEspec(eta,E) * E**2 / (np.exp(E / (kB * Tcell)) - 1)
    integral = scipy.integrate.quad(integrand, E_min, E_max, full_output=1)[0]
    return ((2 * np.pi) / (c0**2 * hPlanck**3)) * integral

def Generated(eta):
    integrand = lambda E : EQEspec(eta,E) * SPhotonsPerTEA(E)
#    integral = scipy.integrate.quad(integrand, E_min, E_max, full_output=1)[0]
    return scipy.integrate.quad(integrand, E_min, E_max, full_output=1)[0]

def current_density(voltage, eta):
    return e * (Generated(eta) - RR0(eta) * np.exp(e * voltage / (kB * Tcell)))

# ## Open-circuit voltage, short-circuit current

def JSC(eta):
    return current_density(0, eta)
def VOC(eta):
    return (kB * Tcell / e) * np.log(Generated(eta) / RR0(eta))
...
def V_mpp(eta):
    ...
    return fmax(lambda voltage : voltage * current_density(voltage, eta))

def J_mpp(eta):
    """ current at max power point """
    return current_density(V_mpp(eta), eta)

def Power(voltage, eta):
    return voltage * current_density(voltage, eta)

Vs = np.linspace(0.1 * V, 2 * V, num=500)
y_values = np.array([Power(x,1) for x in Vs])
plt.figure()
...
def max_efficiency(eta):
    return max_power(eta) / solar_constant